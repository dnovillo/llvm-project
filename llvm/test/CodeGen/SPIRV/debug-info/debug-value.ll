...
; The record reaches MIR ahead of the OpIAdd that defines its value, so the
; handler holds it until that result id exists. Anchor the negative check at
; OpFunction so its region does not depend on where the CHECK-DAG group above
; happened to match.
; CHECK: OpFunction
